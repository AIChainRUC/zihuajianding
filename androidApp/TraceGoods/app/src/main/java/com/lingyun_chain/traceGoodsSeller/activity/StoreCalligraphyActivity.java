...
import com.lingyun_chain.traceGoodsSeller.util.ECDSAUtil;
import com.lingyun_chain.traceGoodsSeller.util.FileProvider7Util;
import com.lingyun_chain.traceGoodsSeller.util.MD5Util;
import com.lingyun_chain.traceGoodsSeller.util.OSutil;
import com.lingyun_chain.traceGoodsSeller.util.StringUtil;
...
import static com.lingyun_chain.traceGoodsSeller.constants.IntentConstants.SELECT_PIC_BY_TACK_PHOTO_IMAGE_DETAIL;
...
public class StoreCalligraphyActivity extends BaseActivity implements View.OnClickListener {
    ...
    private TextView store_imageDetail_submit;
    private EditText companyName_edt;
    private String companyName;
    private EditText productionBatch_edt;
...
private String store_workName = "default";
    private String store_workSize = "default";
    private String creationYear = "default";
    private String classificationWork = "default";
    private String materialWork = "default";
    private String subjectWork = "default";
    ...
    private void inintView() {
        ...
        if (sharedPreference != null) {
            ...
            if (!TextUtils.equals(generatePublicKey, "default") && !TextUtils.equals(generatePrivateKey, "default")) {
                store_text_key.setText(R.string.keyStoreUploaded);
...
//        store_seal_submit.setOnClickListener(this);
        ...
        store_imageDetail_submit=(TextView)findViewById(R.id.store_image_submit_detail);
        store_imageDetail_submit.setOnClickListener(this);
      //  store_assetId = (TextView) findViewById(R.id.store_assetId);
      //  assetId_help = (TextView) findViewById(R.id.assetId_help);
        companyName_edt=(EditText)findViewById(R.id.company_edt);
        companyName_edt.setOnClickListener(this);
        productionBatch_edt=(EditText)findViewById(R.id.productionBatch_edt);
        productionBatch_edt.setOnClickListener(this);
    }
...
//                        "\n");
            ...
            case R.id.store_submit://字画存链
               ...
               // store_workSize = subjectWork_edt.getText().toString().trim();
                ...
                companyName=companyName_edt.getText().toString().trim();
                productionBatch=productionBatch_edt.getText().toString().trim();
...
//                        && !subjectWork.isEmpty()
                        && !TextUtils.equals(featureSeal, "default1")
                        && !TextUtils.equals(picHash, "default1")) {
                    ...
                    if (isStored == true) {//存链成功，仅上传图片
                        ...
                        new AsyUploadImageTask(StoreCalligraphyActivity.this, "AsyUploadImageTask", assetFilePath, jsonData).execute();//图片上传失败，需要再次上传new AsyUploadImageTask(StoreCalligraphyActivity.this, "AsyUploadImageTask", assetFilePath, assetID).execute();//图片上传失败，需要再次上传
...
//                });
//                break;
            case R.id.store_image_submit_detail:
                BaseActivity.requestRuntimePermission(new String[]{Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE}, new PermissionListener() {
                    @Override
                    public void onGranted() {
                        UiUtils.hideInput(StoreCalligraphyActivity.this, store_imageDetail_submit);
                        takePictures(SELECT_PIC_BY_TACK_PHOTO_IMAGE_DETAIL);//打开相机拍照
                    }
                ...
                if (picPath != null && (picPath.endsWith(".png") || picPath.endsWith(".PNG") || picPath.endsWith(".jpg") || picPath.endsWith(".JPG"))) {
                    ...
                    try {
                        ...
                        if (!TextUtils.equals(picHash, "default")) {
                            ...
                            classificationWork_edt.setText("123456789");
                        } else {
...
//                }
                ...
                store_text_key.setText(R.string.keyStoreUploaded);
                store_text_key.setEnabled(false);
            }else if(requestCode == SELECT_PIC_BY_TACK_PHOTO_IMAGE_DETAIL){//二维码相关
                store_imageDetail_submit.setText("图片成功提交");
                store_imageDetail_submit.setTextColor(R.color.colorAccent);
                store_imageDetail_submit.setEnabled(false);
                classificationWork_edt.setText("123456789");
                store_workName="火腿肠";
                store_workName_edt.setText(store_workName);
                creationYear="2018-9-28";//出厂日期
                creationYear_edt.setText(creationYear);
                companyName="浙江金华";
                companyName_edt.setText(companyName);
                productionBatch="2";
                productionBatch_edt.setText(productionBatch);
                classificationWork="123456789";
                classificationWork_edt.setText(classificationWork);
            }
        } else {
            UiUtils.show("拍照失败");
        }
    ...
    public class AsySaveTask extends BaseAsyTask {//提取印章特征值
        ...
        protected void onPostExecute(String s) {
            ...
            if (TextUtils.equals(s, "-1")) {
                ...
                store_seal_submit.setText(R.string.uploadImageAgain);
                ...
                store_seal_submit.setText(R.string.identificationGoodsImageUploaded);
                ...
                UiUtils.show(getString(R.string.identificationGoodsImageError));
            }
        }
    }
    ...
    public class AsyCreateAssetTask extends BaseAsyTask {
        ...
        protected void onPostExecute(String s) {
            ...
            if (TextUtils.equals(s, "-1")) {
                ...
                UiUtils.show(getString(R.string.storeSuccess));
                startActivity(new Intent(StoreCalligraphyActivity.this,StoreResultActivity.class));
                finish();
                //new AsyUploadImageTask(StoreCalligraphyActivity.this, "AsyUploadImageTask", assetFilePath, assetID).execute();//上传图片
                //finish();
            } else {
                UiUtils.show(getString(R.string.storeError));
            }
        }
